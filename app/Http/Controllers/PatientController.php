...
class PatientController extends AppBaseController
{
    ...
    public function store(CreatePatientRequest $request)
    {
        ...
        $exist = Patient::where("ktp","=",$request->input('ktp'))->first();
        if (!empty($exist)) {
         // dd($exist);
          Flash::warning('Patient data already exist.');

        return redirect(route('patients.index'));

        }
        //jika ada
        //maka memberikan peringatan
        
        $trash= Patient::withTrashed()->where("ktp","=",$request->input('ktp'))->first();
        if (!empty($trash)){
            Patient::withTrashed()->where("ktp","=",$request->input('ktp'))->restore();
            
            Flash::success('Patient data restore.');
            return redirect(route('patients.index'));   
        }
        //dd($trash);
